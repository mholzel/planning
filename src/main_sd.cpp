...
bool validStep(Container<T, Rest...> v) {

    const auto a = (v[0] - v[1]) / 0.02;
    const auto a0 = (v[1] - v[2]) / 0.02;
    const auto j = (a - a0) / 0.02;
    if (abs(v[0]) > mphToMps(50))
        return false;
//    if (abs(v[2]) > 0 && abs(a) > 10)
//        return false;
//    if (abs(v[2]) > 0 && abs(j) > 10)
//        return false;
    return true;
}
...
//    const auto map_s = map_s_;
    ...
    std::queue <Waypoint<T>>
            waypoints;
    for (size_t i = 0; i < waypoint_N; ++i) {
...
//                        if (verbose)
                    ...
                    // TODO
                    // Choose the parameter set with the smallest cost
                    T min_cost = 1e50;
                    T best_ds = 0, best_d = 6;
                    for (auto &&ds : {2.5, 5., 10., 15., 21., s_parameterization.v0}) {
                        for (auto &&d  : {2, 6, 10}) {
                            size_t i = 0;
                            solution(i++) = ds;
                            solution(i++) = d;
                            T cost = total_cost.eval(solution);
//                            cout << "params: " << ds << ", " << d << endl;
                            if (std::isfinite(cost) && cost < min_cost) {
                                min_cost = cost;
                                best_ds = ds;
                                best_d = d;
                            }
                    ...
                    vector <T> v = {0, 0, 0};
                    T time_decrease = 0;
                    // TODO. Something seems wrong with prev_t
                    // and for some reason, we keep going outside of the lane.
                    // Finally, we should more strongly reward going fast.
                    for (size_t i = 0; i < waypoint_times.size() - previous_path_x.size(); ++i) {
                        ...
                        auto t = waypoint_times[i] - time_decrease;
                        ...
                        last_x = next_x_vals.empty() ? car_x : next_x_vals.back();
                        last_y = next_y_vals.empty() ? car_y : next_y_vals.back();
                        v[2] = v[1];
                        v[1] = v[0];
                        v[0] = norm(xy[0] - last_x, xy[1] - last_y) / 0.02;
                        const T ratio = .08;
                        while (!validStep(v)) {
                            cout << "Too fast. Truncating timestep from " << t - prev_t << " to ";
                            time_decrease += ratio * (t - prev_t);
                            t -= ratio * (t - prev_t);
                            cout << t - prev_t << endl;
...
//                        time_decrease = waypoint_times[i] - t;
                    ...
                    if (verbose) {
                        cout << "s: " << Map < Matrix < T, Dynamic, 1 >> (next_s.data(), next_s.size()).transpose()
                                                                      << endl;
                        cout << "d: " << Map < Matrix < T, Dynamic, 1 >> (next_d.data(), next_d.size()).transpose()
                                                                      << endl;
                    ...
                    const bool delaying = false;
                    if (delaying) {
                        const auto max_delay_millis = 100;
                        auto delay_millis = max_delay_millis - duration;
                        if (delay_millis < 0) {
                            delay_millis = 0;
                        }
